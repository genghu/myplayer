...
function msSpeakQueued(id, text,note) {
	...
	if (InReplay==false)
	{
		//define an Object(name:value pairs)
		var SpeechObj={};
		SpeechObj.id=id;
		SpeechObj.text=text;
		dialogHistory.push(SpeechObj);
	}
	//replace all the names with the computer agent names.
...
function execute(id, action,note) {
	...
	var char = chars[id];
	//encodeURIComponent encodes special characters. In addition, it encodes the
	//following characters: , / ? : @ & = + $ #
	//https://apiwest.x-in-y.com/app/animate.jpeg
	savedURL = animateBase + "?character=" + char.character  +
		...
		//behind the scenes. This means that it is possible to update parts
...
//This function is used to draw rectangle, circles, and a triangle under the div block.
...
//used to decide the sizes of the head, bust, body, and cartoon
//and put the charactors' face, format, voice, and size in o
