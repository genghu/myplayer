  ...
  if (itemAct.mediaTypeXML != null) {
    ...
    if (itemAct.mediaTypeXML["#text"] == "MovieOnly") {
      OtherActions = '<cmd mv="' + aitem.PageConfig.MediaURLXML["#text"] + '"/>';
			...
			VideoxmlData = aitem.PageConfig.MediaURLXML;
			console.log("####### Get the VideoxmlData ---------> " + VideoxmlData["#text"]);
    }
    ...
    for (var i = 0; i < item.length; i++) {
      try {
        ...
        if (atype.type == "scene") {
          ...
          console.log('Testing ### -----> '+ atype.type);
        } else {
...
function StartTutoring(MODE, jsonOfXml) {
  ...
  if (MODE == "ID") {
    Status = "ID";
    IDDialog(jsonOfXml);
		/*suppress the display of MediaContainer. If not doing this,
		the MediaContainer will be cover the youtube video, and users will
		not be able to click the video. */
